...
const Invite_friends = ({ t }) => {
  ...
  const clickToCopy = event => {
    ...
    if (navigator.clipboard !== undefined) {//Chrome
      navigator.clipboard.writeText(user.refer_code).then(function () {
      }, function (err) {
      });
  ...
  return (
    <React.Fragment>
      ...
      <TopHeader />
      <Header />
      <Breadcrumb title="Invite Friends" content="Home" contentTwo="Invite Friends" />
      <div className="Invite__friends">
        <div className="container">
          <div className="row morphisam">
            <div className="col-lg-6 col-md-12 col-12">
              <div className="invite_friends_image">
                <img src={process.env.PUBLIC_URL + `/images/refer-earn/refer-earn.png`} alt="invite friends" />
              </div>
            </div>
            <div className="col-lg-6 col-md-12 col-12 border_line justify-content-center align-items-center d-flex flex-column text-center">
              <div className="refer_earn_title">
                <h3><b> {t('Refer and Earn')}</b></h3>
                <p>{t('Refer and Earn Text')}</p>
                <h5><b>{t('Your Referral Code')}</b></h5>
              </div>
              <div className="copy__referal">
                ...
                <button className="copy-btn">
                  <span className="icon1">
                    <Link to={""} onClick={(event) => { clickToCopy(event) }}> <i><FaRegCopy /></i>&nbsp;{copyState ? t("Copied") : t('Tap to Copy')}</Link>
                  </span>
                </button>
              </div>
              <div className="invite__now">
                <p><b>{t('Invite Now')}:</b></p>
