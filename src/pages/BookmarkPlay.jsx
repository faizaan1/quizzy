...
const BookmarkPlay = ({ t }) => {
	...
	const getNewQuestions = () => {
		api.getBookmark(1).then(response => {
			if (!response.error) {
				var questions = response.data.map((data) => ({ ...data, isBookmarked: false, selected_answer: '', isAnswered: false }));
						...
						<div className="whitebackground">
							{
								(() => {
									if (showBackButton) {
										return <div className="dashoptions">
										...
										return questions && questions.length > 0
											?
											<Question questions={questions} timerSeconds={TIMER_SECONDS} onOptionClick={handleAnswerOptionClick} onQuestionEnd={onQuestionEnd} showLifeLine={false} showBookmark={false} setCoinScoreOnEnd={false}/>
											:
