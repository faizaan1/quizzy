...
const Profile = ({ t }) => {
    ...
    const [statistics, setStatistics] = useState({ correct_answers: 0, questions_answered: 0 });
    ...
    const formSubmit = (e) => {
        ...
        setLoad(true);
        if (!config.demo) {
            api.updateUserProfile(profile.name, profile.mobile).then((response) => {
                if (!response.error) {
                    var user = JSON.parse(localStorage.getItem('user'));
                    var data = { ...user, 'name': response.data.name, 'mobile': response.data.mobile };
    ...
    const handleImageChange = (e) => {
        ...
        if (!config.demo) {
            api.updateUserProfileImage(e.target.files[0]).then((response) => {
                if (!response.error) {
                    setProfile(values => ({ ...values, 'profile': response.data.profile }));
                } else {
                    toast.error(response.error);
                }
            })
                                ...
                                <div className="col-xl-5 col-lg-5 col-md-12 col-12 ">
                                    <div className="card main__profile pt-5 d-flex justify-content-center align-items-center">
                                        <div className="prop__image">
                                            <img src={profile.profile ? profile.profile : process.env.PUBLIC_URL + "/images/user.png"} alt="profile" id='user_profile' />
                                            <div className="select__profile">
                                                <input type="file" name='profile' id="file" onChange={handleImageChange} />
                                                <label htmlFor="file"> <em><FaCamera /></em></label>
                                        ...
                                        </div>
                                        {
                                            (profile.mobile) ?
                                                <div className="mobile__number">
                                                    <span><i><FaPhoneAlt /></i><p>{profile.mobile}</p></span>
                                                </div>
                                                : ''
                                        }

                                        {
                                            (profile.email) ?
                                                <div className="email__id">
                                                    <span><i><FaEnvelope /></i><p>{profile.email}</p></span>
                                                </div>
                                                : ''
                                        }
                                    </div>
                                            ...
                                            <div className="col-md-6 col-12">
                                                <label htmlFor="fullName">
                                                    <input
                                                        ...
                                                        required
                                                    />
                                                    <i className='custom-icon'><FaUserCircle /></i>
                                                </label>
                                            </div>
                                            <div className="col-md-6 col-12">
                                                <label htmlFor="mobilenumber">
                                                    ...
                                                    />
                                                    <i className='custom-icon'><FaMobileAlt /></i>
                                                </label>
                                            </div>
                                        </div>
                                        <button className="btn btn-primary text-uppercase" type="submit" value="submit" name="submit" id="mc-embedded-subscribe">{!load ? t('Submit') : t('Please Wait')}</button>
                                                ...
                                                <div className="col-md-6 col-12">
                                                    <div className="bookmark__content">
                                                        <NavLink to={"/Bookmark"} className="w-100 d-block">
                                                            ...
                                                            <i className='custom-icon'><FaRegBookmark /></i>
                                                        </NavLink>
                                                    </div>
                                                </div>
                                                <div className="col-md-6 col-12">
                                                    <div className="Invite_friends__content">
                                                        <NavLink to={"/Invite_friends"} className="w-100 d-block">
                                                            <span>{t('Invite Friends')}</span>
                                                            <i className='custom-icon'><FaEnvelopeOpenText /></i>
                                                        </NavLink>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div className="col-md-6 col-12">
                                                    <div className="Delete_account__content">
                                                        <NavLink to={""} className="w-100 d-block" onClick={deleteAccountClick}>
                                                            ...
                                                            <i className='custom-icon'><FaTrashAlt /></i>
                                                        </NavLink>
                                                    </div>
                                                </div>
                                                <div className="col-md-6 col-12">
                                                    <div className="Logout__content">
                                                        <NavLink to={""} onClick={handleSignout} className="w-100 d-block">
                                                            <span>{t('Logout')}</span>
                                                            <i className='custom-icon'><FaSignOutAlt /></i>
                                                        </NavLink>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div className="col-md-6 col-12">
                            <div className="quiz_details morphism">
                                ...
                                <ul className='quiz_details_inner'>
                                    <li>{t('Rank')}<span className="badge badge-pill custom_badge">{profile.all_time_rank}</span></li>
                                    <li>{t('Score')}<span className="badge badge-pill custom_badge">{profile.all_time_score}</span></li>
                                    <li>{t('Coins')}<span className="badge badge-pill custom_badge">{profile.coins}</span></li>
                                </ul>
                            </div>
                        </div>
                        <div className="col-md-6 col-12">
                            <div className="questions_details morphism">
                                ...
                                <ul className="questions_details_inner">
                                    <li>{t('Attempted')}<span className='badge badge-pill custom_badge'>{statistics.questions_answered}</span></li>
                                    <li>{t('Correct')}<span className='badge badge-pill custom_badge'>{statistics.correct_answers}</span></li>
                                    <li>{t('Incorrect')}<span className='badge badge-pill custom_badge'>{statistics.questions_answered && statistics.correct_answers && parseInt(statistics.questions_answered) - parseInt(statistics.correct_answers)}</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
