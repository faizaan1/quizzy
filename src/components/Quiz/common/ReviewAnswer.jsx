...
function ReviewAnswer({ questions, goBack, t }) {
            ...
            if (result.isConfirmed) {
                api.reportQuestion(question_id, result.value).then((response) => {
                    if (!response.error) {
                        Swal.fire(
                            t('Success'),
                            t('Question Reported successfully'),
                            'success'
                        )
                    } else {
                        Swal.fire(
    ...
    return (
        <React.Fragment>
            <div className="text-center">
                <h4 className='text-white'>{t('Review Answers')}</h4>
            </div>
            <div className="inner__headerdash">
                ...
                <div className="total__out__leveldata coinsdata">
                    <h5 className="inner__total-leveldata text-white">{currentQuestion + 1} | {questions.length}</h5>
                </div>
                <div className="bookmark_area" >
                    <button title="Report Question" className="btn bookmark_btn text-white " onClick={() => reportQuestion(questions[currentQuestion].id)}><FaExclamationTriangle className='fa-2x' /></button>
                </div>
            </div>
                    ...
                    : ''}
                {
                    (!questions[currentQuestion].selected_answer) ?
                        <div className="text-end">
                            <span className='text-white'>*{t('Not Attempted')}</span>
                        </div>
                        : ''
                }
            </div>
            ...
            <div className="dashoptions">
                <div className="fifty__fifty">
                    <button className="btn" onClick={previousQuestion} disabled={disablePrev}>&lt;</button>
                </div>
                <div className="resettimer">
                    <button className="btn" onClick={goBack}>{t('Back')}</button>

                </div>
                <div className="skip__questions">
                    <button className="btn" onClick={nextQuestion} disabled={disableNext}>&gt;</button>
                </div>
            </div>
